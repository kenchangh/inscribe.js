...
/*
 * For compressing strings to be stored in localStorage
 */
...
/*
 * For a better API of localStorage
 * Asynchronous and serializes from JSON.
 */
var storage = {};
...
storage.get = function getStorage(key, callback) {
  setTimeout(function() {
    ...
    callback(value); // mandate for a callback 
  })
};
...
/*
 * The start of the inscribe.js code.
 */

var inscribe = {};
inscribe.store = storeViews;
...
function fetchView(url) {
  ...
  function processHTML(html) {
    ...
    internalizeImages($html, function($html) {
      internalizeJS($html, function($html) {
        internalizeCSS($html, function($html) {
 ...
 * To embed external assets into the HTML string.
 *
 * Element arrays made from running jQuery on HTML string is passed around.
 */
